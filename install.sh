...
echo 'alias btc="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "http://github.com/arkdeployer/core" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'http://github.com/arkdeployer/core'"
...
rm -rf "$HOME/.config/@btc"
rm -rf "$HOME/.config/@piechain"
rm -rf "$HOME/.config/btc-core"
